...
## 🏥 API Reference: Patients Domain

Required endpoints to support the Frontend Dashboard (`/dashboard/patients`).
...
### 2. Search Patients
- **GET** `/api/v1/prm/patients/search`
- **Query Params**:
  - `query` (string)
  - `type` (`phone` | `name` | `mrn`)
- **Response**: List of matching patients.

### 3. Patient Details & 360 View
- **GET** `/api/v1/prm/patients/{id}` - Basic details.
- **GET** `/api/v1/prm/patients/{id}/360` - Aggregated view (Patient + Appointments + Journeys + Tickets).

### 4. Patient Management
- **POST** `/api/v1/prm/patients` - Create new patient.
- **PATCH** `/api/v1/prm/patients/{id}` - Update patient details.
- **DELETE** `/api/v1/prm/patients/{id}` - Archive/Delete patient.
